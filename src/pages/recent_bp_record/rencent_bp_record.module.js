 ...
 */
const GET_R_BP = '/cdm/user/pressure/last/list';//查询医助信息
...
export default {
  ...
  actions: {
    ...
    async searchBaseInfo({commit,state},data){
      ...
      const searchInfo = await request.get(GET_R_BP,{params:state.searchInfo});
      searchInfo.data.total_count && commit('SAVE_ALL_PAGE_NUM',searchInfo.data.total_count);
      searchInfo.data.pressure_list && commit('SAVE_HOME_DATA', searchInfo.data.pressure_list);
    }
  },
